...
```bash
test -f youtrack-report/.env \
  && grep -q "^YOUTRACK_URL=https" youtrack-report/.env \
  && grep -q "^YOUTRACK_TOKEN=[^Y]" youtrack-report/.env \
  && echo "✅ .env 已設定" \
  || echo "❌ .env 不存在或尚未填入真實值"
```

> 注意：不要用 `cat` 顯示 `.env` 內容，避免 token 出現在終端記錄或螢幕分享中。
