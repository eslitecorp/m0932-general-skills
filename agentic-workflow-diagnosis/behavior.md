...
**三態回答「有沒有在用」，⛔ 答不出「用了有沒有用」。** 取數時一併記下索引器的能力深度 ——
至少 `embeddings` 數與搜尋模式（語意 vs 純檔名／路徑 FTS），以及它涵蓋哪些副檔名。
⚠️ **這一版只要求記錄與在報告裡分開陳述，⛔ 不改硬旗標的判定式**；
判準本體的變更要另開一次預先登記（沿用 v0.3.2 對指標 1 的處置）。

兩個已知會讓繞道率被高估的形態，報告要指名、不得整筆歸成行為問題：

| 形態 | 為什麼繞道是合理的 |
|---|---|
| 索引只做名稱／路徑比對（無 embeddings） | 「某段邏輯在哪」這類查詢它答不出來，grep 答得出來 |
| 探勘目標的副檔名不在索引器涵蓋範圍 | `is_code_target()` 收 repo root 底下的**任何**檔，含 `.md`；索引器可能只吃程式碼，兩者範圍不同 |

〔事證：某台索引器 `embeddings` 為 0、自陳「covers names, paths and signatures, not source text」；
同一台的一個純文件 repo 有 213 個 `.md`，圖譜節點 0〕
...
| 段落 | 狀態 |
|---|---|
| 指標 1、2、3、5 的計算式與陷阱 | ✅ **已在多台機器實地驗證**（一台 207 session／87,765 行、一台 7 session／2,384 行，皆 0 解析失敗） |
| 指標 4 的兩個 ⛔ | ✅ 已實測否證（774h span、184.6h 中位拖尾），並有 fixture 測試覆蓋 |
| `index_use` 的三態 | ✅ 某台閒置型機是「有設定、process 活著、0 次呼叫」的實例，這一態在 v0.1 被誤壓成硬旗標 |
| **索引器的能力深度** | ❌ **判準待補，本版只記錄不判定**。實例：`index_state` `CONFIGURED`、`index_use` 63、三態全部正常，但 `embeddings` 為 0、只做名稱與路徑比對 —— 繞道有一部分是能力限制。⛔ 改判準要另開預先登記 |
| **分母與索引器涵蓋範圍錯配** | ❌ **量不出佔比**。`is_code_target()` 收 repo root 底下任何檔（含 `.md`），索引器可能只吃程式碼；目前拆不出繞道量裡有多少是索引結構上就不涵蓋的目標 |
...
| 語料截斷偵測 | ✅ 實測命中（某機語料涵蓋 106 天 vs 保留期 30 天，`list_sessions` 另有 3 個月的落差） |

⛔ 沒驗證過的放待累積，**不要憑空杜撰**。
**本表由維護者維護，套用本 skill 不需要回填。**
判準不合、或發現值得新增的檢核方式 → 依 [CONTRIBUTING.md](../CONTRIBUTING.md) 流程開 Issue
（`skill-proposal` template），由維護者統整後更新。
